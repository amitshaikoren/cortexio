...
from cortexio.parsers import parse
...
def test_parse_pose_from_cli(data_dir, tmp_path):
    ...
    command = f"python -m cortexio.parsers parse 'pose' {source_path}; cat {source_path} > {result_path}"
    os.system(command)
    ...
    assert parsing_result['pose_translation_x'] == 0.123
    assert parsing_result['pose_translation_y'] == 0.456
    assert parsing_result['pose_translation_z'] == 0.789

    assert parsing_result['pose_rotation_w'] == 0
    assert parsing_result['pose_rotation_x'] == 0.123
    assert parsing_result['pose_rotation_y'] == 0.456
    assert parsing_result['pose_rotation_z'] == 0.789
